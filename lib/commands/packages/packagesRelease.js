import {
  asyncExec,
  askConfirm,
  askOptions,
  getLernaJson,
  logger,
  checkBranchUpToDate,
  checkIfAllowedBranch,
  getBranchOptions,
  getCurrentBranch,
  isPrereleaseBranch
} from '../../util/index.js';

import { packagesBuild } from './packagesBuild.js';

const makePrepare = async (argv) => {
  if (argv.clean) {
    await asyncExec('npx lerna clean --yes');
  }
  if (argv.bootstrap) {
    await asyncExec('npx lerna bootstrap -- --legacy-peer-deps');
  }
};

const revertPublish = async () => {
  const { version } = getLernaJson();
  await asyncExec('git reset --hard HEAD~1');
  await asyncExec(`git tag -d v${version}`);
};

const tryPublish = async (versionFlags = [], publishFlags = []) => {
  try {
    await asyncExec(`lerna version ${versionFlags.join(' ')}`);
    await asyncExec('git push --tags');
    await asyncExec(`npx lerna publish from-package ${publishFlags.join(' ')}`);
  } catch (e) {
    await revertPublish();
    logger.error(e);
  }
};

const makePrereleasePublish = async (currentBranch) => {
  const { prerelease: preid, channel: distTag } = getBranchOptions(currentBranch);

  const versionFlags = [
    '--conventional-prerelease',
    `--preid ${preid}`,
    '--no-push',
    '--exact',
    '--yes'
  ];
  const publishFlags = [
    `--dist-tag ${distTag}`
  ];

  await tryPublish(versionFlags, publishFlags);
};

const makeReleasePublish = async () => {
  const { version } = getLernaJson();
  const devRegex = /^v\d+\.\d+\.\d+-.*$/;

  const needGraduate = devRegex.test(version);

  if (needGraduate) {
    await asyncExec(`git tag -d $(git tag -l | grep -E '${devRegex.toString()}')`);
  }

  const versionFlags = [
    ...(needGraduate ? ['--conventional-graduate'] : []),
    '--no-push',
    '--exact',
    '--yes'
  ];

  await tryPublish(versionFlags, []);

  if (needGraduate) {
    await asyncExec('git pull --tags');
  }
};

const makeCheck = async (currentBranch) => {
  try {
    await checkIfAllowedBranch(currentBranch);
    await checkBranchUpToDate(currentBranch);
  } catch (e) {
    logger.error(e);
  }
};

const makePublish = async (currentBranch, argv) => {
  logger.start('Preparing for release...');
  await makePrepare(argv);
  await packagesBuild.handler();
  logger.succeed();

  logger.start('Make release...');
  if (isPrereleaseBranch(currentBranch)) {
    await makePrereleasePublish(currentBranch);
  } else {
    await makeReleasePublish(currentBranch);
  }
  logger.succeed();
};

// //////////////////////////////

const command = 'release';

const commandOptions = {
  bootstrap: {
    type: 'boolean',
    alias: 'bs',
    describe: 'Bootstrap packagesCommand',
    default: true
  },
  clean: {
    type: 'boolean',
    alias: 'cl',
    describe: 'Clean packagesCommand',
    default: true
  }
};

const describe = 'Release and publish frameworkCommand packagesCommand';

const builder = (yargs) => yargs
  .options({
    ...commandOptions,
    yes: {
      alias: 'y',
      describe: 'Confirm packagesCommand release'
    }
  })
  .example([
    [
      `$0 ${command}`,
      'Make default packagesCommand publish'
    ],
    [
      `$0 ${command} --prerelease`,
      'Make prerelease packagesCommand publish with \'test\' identifier'
    ],
    [
      `$0 ${command} --prerelease --preid next`,
      'Make prerelease packagesCommand publish with \'next\' identifier'
    ]
  ]);

const handler = async (argv) => {
  const _argv = argv.interactive ? await askOptions(commandOptions) : argv;

  const currentBranch = await getCurrentBranch();

  logger.start('Check brunch ...');
  await makeCheck(currentBranch);
  logger.succeed('All check passed.');

  if (_argv.yes) {
    await makePublish(currentBranch, _argv);
  } else {
    await askConfirm('Release packagesCommand?\n', () => makePublish(currentBranch, _argv));
  }
};

export const packagesRelease = {
  command,
  describe,
  builder,
  handler
};
